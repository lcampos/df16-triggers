trigger SessionSpeakerTrigger on Session_Speaker__c (before insert, before update, after insert) {
    ...
    if (Trigger.isBefore && (Trigger.isInsert || Trigger.isUpdate)) {
        ...
        List<Session__c> related_sessions = [SELECT Id, Date__c from Session__c WHERE ID IN :requested_bookings.keySet()];
        for(Session__c related_session : related_sessions) {
            requested_bookings.put(related_session.Id,related_session.Date__c);
        }

        //get related speaker sessions to check against
        List<Session_Speaker__c> related_speakers = [SELECT ID, Speaker__c, Session__c, Session__r.Date__c from Session_Speaker__c WHERE Speaker__c IN :speakerIds];
